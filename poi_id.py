...
def get_decision_tree():
    ...
    from sklearn import grid_search
    param_grid = {'min_samples_split': [2,3,4,5,6],'max_features':[3]}
...
def get_random_forest():
    ...
    param_grid = {'n_estimators': [10,50],'min_samples_split':[2,3,4,5],'max_features':[3]}
...
def draw_plots(data_dict):
    ...
    for s,idx in zip(data,range(len(data))):
        plt.scatter(data[idx][1],data[idx][0],c='R')
    ...
    for s,idx in zip(data,range(len(data))):
        plt.scatter(data[idx][2],data[idx][0],c='R')
    plt.xlabel('Total Stocks')
    plt.ylabel('POI')
...
features = ['poi','salary','bonus', 'total_stock_value', 'total_payments','other','long_term_incentive','deferred_income', 'exercised_stock_options','from_poi_to_this_person','from_this_person_to_poi','to_messages','from_messages','shared_receipt_with_poi']
...
print 'Number of Features %s'% len(data_dict.get('METTS MARK').keys())

print 'List of Features %s'% data_dict.get('METTS MARK').keys()

print 'Size %d'%len(data_dict)
...
print 'Total Number of POI = %d'%ctr

### Task 2: Remove outliers
draw_plots(data_dict)
...
for key in data_dict:
    ...
    data_dict[key]['overall_total']= data_dict[key]['total_payments']+data_dict[key]['total_stock_value']
...
#features_list.append('overall_total')
...
enron_classifier='Decision_Tree'

if enron_classifier == 'GaussianNB':
    clf = get_gaussianNB()
elif enron_classifier == 'Decision_Tree':
  clf = get_decision_tree()
elif enron_classifier == 'Random_Forest':
